...
func _ready():
	...
	mat.set_shader_parameter('view_tex', $imagem_svp.get_texture())
	$imagem_svp/cam_spv.cull_mask = render_layer
	set_child_visual_layer_recursive(self)

# Called every frame. 'delta' is the elapsed time since the previous frame.
func _process(delta):
	var rect = get_viewport().size
	$imagem_svp.get_viewport().size = rect
	
	$imagem_svp/origem.global_position = global_position
	$imagem_svp/origem.global_rotation = global_rotation
